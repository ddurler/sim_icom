...
impl Database {
    ...
    #[allow(dead_code)]
    pub fn get_u16_from_word_address(&self, word_address: WordAddress) -> u16 {
        let vec_u8 = self.get_vec_u8_from_word_address(word_address, 2);
    ...
    #[allow(dead_code)]
    pub fn set_u16_to_word_address(&mut self, word_address: WordAddress, value: u16) {
    ...
    pub fn get_u16_from_id_tag(&self, id_tag: &IdTag) -> u16 {
        match self.get_tag_from_id_tag(id_tag) {
            Some(id_tag) => self.get_u16_from_word_address(id_tag.word_address),
            None => u16::default(),
        }
    }
    ...
    pub fn set_u16_to_id_tag(&mut self, id_tag: &IdTag, value: u16) {
        if let Some(id_tag) = self.get_tag_from_id_tag(id_tag) {
            self.set_u16_to_word_address(id_tag.word_address, value);
        }
    }
}
...
mod tests {
    ...
    fn test_address_u16() {
        ...
        assert_eq!(db.get_u16_from_word_address(addr), u16::default());

        for value in [0_u16, 1_000_u16, 50_000_u16] {
            db.set_u16_to_word_address(addr, value);
            assert_eq!(db.get_u16_from_word_address(addr), value);

            db.set_u16_to_word_address(addr, value + 1);
            assert_eq!(db.get_u16_from_word_address(addr), value + 1);
        }
    }
