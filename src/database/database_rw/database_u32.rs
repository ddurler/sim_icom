...
impl Database {
    ...
    #[allow(dead_code)]
    pub fn get_u32_from_word_address(&self, word_address: WordAddress) -> u32 {
    ...
    #[allow(dead_code)]
    pub fn set_u32_to_word_address(&mut self, word_address: WordAddress, value: u32) {
    ...
    pub fn get_u32_from_id_tag(&self, id_tag: &IdTag) -> u32 {
        match self.get_tag_from_id_tag(id_tag) {
            Some(id_tag) => self.get_u32_from_word_address(id_tag.word_address),
            None => u32::default(),
        }
    }
    ...
    pub fn set_u32_to_id_tag(&mut self, id_tag: &IdTag, value: u32) {
        if let Some(id_tag) = self.get_tag_from_id_tag(id_tag) {
            self.set_u32_to_word_address(id_tag.word_address, value);
        }
    }
}
...
mod tests {
    ...
    fn test_address_u32() {
        ...
        assert_eq!(db.get_u32_from_word_address(addr), u32::default());

        for value in [0_u32, 1_000_u32, 1_000_000_u32] {
            db.set_u32_to_word_address(addr, value);
            assert_eq!(db.get_u32_from_word_address(addr), value);

            db.set_u32_to_word_address(addr, value + 1);
            assert_eq!(db.get_u32_from_word_address(addr), value + 1);
        }
    }
