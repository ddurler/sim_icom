...
impl Database {
    ...
    #[allow(dead_code)]
    pub fn get_i16_from_word_address(&self, word_address: WordAddress) -> i16 {
    ...
    #[allow(dead_code)]
    pub fn set_i16_to_word_address(&mut self, word_address: WordAddress, value: i16) {
    ...
    pub fn get_i16_from_id_tag(&self, id_tag: &IdTag) -> i16 {
        match self.get_tag_from_id_tag(id_tag) {
            Some(id_tag) => self.get_i16_from_word_address(id_tag.word_address),
            None => i16::default(),
        }
    }
    ...
    pub fn set_i16_to_id_tag(&mut self, id_tag: &IdTag, value: i16) {
        if let Some(id_tag) = self.get_tag_from_id_tag(id_tag) {
            self.set_i16_to_word_address(id_tag.word_address, value);
        }
    }
}
...
mod tests {
    ...
    fn test_address_i16() {
        ...
        assert_eq!(db.get_i16_from_word_address(addr), i16::default());

        for value in [-10_000_i16, -1_000_i16, 0_i16, 1_000_i16, 10_000_i16] {
            db.set_i16_to_word_address(addr, value);
            assert_eq!(db.get_i16_from_word_address(addr), value);

            db.set_i16_to_word_address(addr, value + 1);
            assert_eq!(db.get_i16_from_word_address(addr), value + 1);
        }
    }
