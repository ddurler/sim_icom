...
impl Database {
    ...
    #[allow(dead_code)]
    pub fn get_i8_from_address(&self, addr: u16) -> i8 {
    ...
    #[allow(dead_code)]
    pub fn set_i8_to_address(&mut self, addr: u16, value: i8) {
    ...
    pub fn get_i8_from_id_tag(&self, id_tag: &IdTag) -> i8 {
        match self.get_tag_from_id_tag(id_tag) {
            Some(id_tag) => self.get_i8_from_address(id_tag.address),
            None => i8::default(),
        }
    }
    ...
    pub fn set_i8_to_id_tag(&mut self, id_tag: &IdTag, value: i8) {
        if let Some(id_tag) = self.get_tag_from_id_tag(id_tag) {
            self.set_i8_to_address(id_tag.address, value);
        }
    }
}
...
mod tests {
    ...
    fn test_address_i8() {
        ...
        assert_eq!(db.get_i8_from_address(addr), i8::default());

        for value in [-100_i8, -10_i8, 0_i8, 10_i8, 100_i8] {
            db.set_i8_to_address(addr, value);
            assert_eq!(db.get_i8_from_address(addr), value);

            db.set_i8_to_address(addr, value + 1);
            assert_eq!(db.get_i8_from_address(addr), value + 1);
        }
    }
